/*
 * NX-Shell NRO Forwarder
 * Based on nx-hbloader by yellows8 (hacbrewpack sample_hbmenu_loader)
 * Updated for modern libnx API
 * 
 * This is a minimal forwarder that:
 * 1. Reads the target NRO path from its RomFS (nextNroPath file)
 * 2. Loads and executes the target NRO with proper homebrew environment
 * 
 * Licensed under ISC License (same as original nx-hbloader)
 */
...
#define FORWARDER_VERSION "1.0.0"

const char g_noticeText[] =
    "NX-Shell Forwarder " FORWARDER_VERSION "\0"
    "NRO forwarder for installed homebrew applications.";
...
static void loadNextPathFromRomfs(void)
{
    // Initialize romfs to read our embedded target path
    Result rc = romfsInit();
    if (R_FAILED(rc)) {
        // If no romfs, we can't load anything
        return;
    }
    
    // Read nextNroPath
    FILE* f = fopen("romfs:/nextNroPath", "r");
    if (f) {
        size_t len = fread(g_nextNroPath, 1, sizeof(g_nextNroPath) - 1, f);
        g_nextNroPath[len] = '\0';
        // Trim any trailing whitespace/newlines
        while (len > 0 && (g_nextNroPath[len-1] == '\n' || g_nextNroPath[len-1] == '\r' || g_nextNroPath[len-1] == ' ')) {
            g_nextNroPath[--len] = '\0';
        }
        fclose(f);
    }
    
    // Read nextArgv (optional, defaults to nextNroPath)
    f = fopen("romfs:/nextArgv", "r");
    if (f) {
        size_t len = fread(g_nextArgv, 1, sizeof(g_nextArgv) - 1, f);
        g_nextArgv[len] = '\0';
        while (len > 0 && (g_nextArgv[len-1] == '\n' || g_nextArgv[len-1] == '\r' || g_nextArgv[len-1] == ' ')) {
            g_nextArgv[--len] = '\0';
        }
        fclose(f);
    } else {
        // Default argv to the NRO path
...
void __appInit(void)
{
    ...
    // Load target NRO path from our romfs
    loadNextPathFromRomfs();
}
...
static void procHandleReceiveThread(void* arg)
{
    ...
    // Clear TLS for IPC receive (matches reference implementation)
...
// Gets the PID of the process with application_type==APPLICATION in the NPDM
...
// Gets the control.nacp for the current title id
static void getIsAutomaticGameplayRecording(void) {
    if (hosversionAtLeast(5,0,0) && g_isApplication) {
        ...
        // VideoCapture value 2 means Automatic
        if (R_SUCCEEDED(rc) && g_applicationControlData.nacp.video_capture == 2)
...
{
    static Thread t;  // Must be static - persists after function returns
    Result rc;
...
void loadNro(void)
{
    ...
    {
        // NRO exited and no next path specified
        // For a forwarder running as an installed title, we can either:
        // 1. Load hbmenu as fallback (like the reference implementation)
        // 2. Exit (which may show error on some firmware)
        
        // Try to load hbmenu as fallback - this is what nx-hbloader does
        FILE* f = fopen("sdmc:/hbmenu.nro", "rb");
        if (f != NULL) {
            fclose(f);
            strcpy(g_nextNroPath, "sdmc:/hbmenu.nro");
            strcpy(g_nextArgv, "sdmc:/hbmenu.nro");
            // Continue to load hbmenu
        } else {
            // No hbmenu available - just exit
            // Note: This may show "software closed" on some firmware
            svcExitProcess();
        }
    }
    ...
    bool has_mod0 = false;
    if (start->mod_offset > 0 && start->mod_offset <= (total_size-0x24))
        has_mod0 = *(uint32_t*)(nrobuf + start->mod_offset) == 0x30444F4D;
    ...
    rc = svcSetProcessMemoryPermission(
        g_procHandle, map_addr + header->segments[0].file_off, header->segments[0].size, Perm_Rx);
    if (R_FAILED(rc))
    ...
    static ConfigEntry entries[] = {
        ...
        { EntryType_SyscallAvailableHint, 0, {UINT64_MAX, 0x9fc1fff0007ffff} },
    ...
    nroEntrypointTrampoline((u64) entries, (u64)-1, entrypoint);
}
