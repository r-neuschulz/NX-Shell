 ...
 * 
 * This is a minimal homebrew loader that runs as an installed title (NSP).
 * When launched from the home menu, it loads and executes NX-Shell.nro
 * from the SD card, providing full application memory access.
 * 
 * Based on nx-hbloader by switchbrew.
 */
...
// NRO search paths (in priority order)
#define HBMENU_NRO      "sdmc:/hbmenu.nro"
#define DEFAULT_NRO     "sdmc:/switch/NX-Shell/NX-Shell.nro"
#define ALT_NRO_1       "sdmc:/switch/NX-Shell.nro"
#define ALT_NRO_2       "sdmc:/NX-Shell.nro"

const char g_noticeText[] =
    "NX-Shell Forwarder " VERSION
    "\0"
...
void loadNro(void) {
    ...
    if (g_nroSize > 0) {
        ...
        rc = svcUnmapProcessCodeMemory(g_procHandle, g_nroAddr + header->segments[0].file_off,
            ((u64)g_heapAddr) + header->segments[0].file_off, header->segments[0].size);
        if (R_FAILED(rc)) diagAbortWithResult(MAKERESULT(Module_HomebrewLoader, 24));

        rc = svcUnmapProcessCodeMemory(g_procHandle, g_nroAddr + header->segments[1].file_off,
            ((u64)g_heapAddr) + header->segments[1].file_off, header->segments[1].size);
        if (R_FAILED(rc)) diagAbortWithResult(MAKERESULT(Module_HomebrewLoader, 25));

        rc = svcUnmapProcessCodeMemory(g_procHandle, g_nroAddr + header->segments[2].file_off,
    ...
    // Find NX-Shell.nro - check multiple paths
    if (g_nextNroPath[0] == '\0') {
        ...
        } else if (access(ALT_NRO_1, F_OK) != -1) {
            memcpy(g_nextNroPath, ALT_NRO_1, sizeof(ALT_NRO_1));
            memcpy(g_nextArgv, ALT_NRO_1, sizeof(ALT_NRO_1));
        } else if (access(ALT_NRO_2, F_OK) != -1) {
            memcpy(g_nextNroPath, ALT_NRO_2, sizeof(ALT_NRO_2));
            memcpy(g_nextArgv, ALT_NRO_2, sizeof(ALT_NRO_2));
        } else {
            // Fallback to hbmenu if NX-Shell not found
    ...
    rc = svcSetProcessMemoryPermission(g_procHandle, (u64)map_addr + header->segments[0].file_off,
        header->segments[0].size, Perm_R | Perm_X);
    if (R_FAILED(rc)) diagAbortWithResult(MAKERESULT(Module_HomebrewLoader, 19));

    rc = svcSetProcessMemoryPermission(g_procHandle, (u64)map_addr + header->segments[1].file_off,
        header->segments[1].size, Perm_R);
    if (R_FAILED(rc)) diagAbortWithResult(MAKERESULT(Module_HomebrewLoader, 20));

    rc = svcSetProcessMemoryPermission(g_procHandle, (u64)map_addr + header->segments[2].file_off,
        rw_size, Perm_Rw);
    if (R_FAILED(rc)) diagAbortWithResult(MAKERESULT(Module_HomebrewLoader, 21));
